...
function makeit_debootstrap() {
    ...
    sudo debootstrap --variant=minbase --arch=${arch} ${vers} build http://${cacher}ftp.de.debian.org/debian/
    ...
    echo "RUN DEBCONF_FRONTEND=noninteractive DEBIAN_FRONTEND=noninteractive apt-get -q --yes upgrade" >> Dockerfile
...
makeit_debootstrap i386 stretch
makeit_debootstrap amd64 stretch
